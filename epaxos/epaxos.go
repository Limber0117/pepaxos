...
import (
	...
	"github.com/tarcisiocjr/pepaxos/bloomfilter"
	"github.com/tarcisiocjr/pepaxos/dlog"
	"github.com/tarcisiocjr/pepaxos/epaxosproto"
	"github.com/tarcisiocjr/pepaxos/fastrpc"
	"github.com/tarcisiocjr/pepaxos/genericsmr"
...
func NewReplica(id int, peerAddrList []string, thrifty bool, exec bool, dreply bool, beacon bool, durable bool, batchWait int, execThreads int, listSize int64, execParallel bool) *Replica {
	r := &Replica{
		genericsmr.NewReplica(id, peerAddrList, thrifty, exec, dreply, listSize),
	...
	// for bf_PT = 1; math.Pow(2, float64(bf_PT))/float64(r.batchWait) < BF_M_N; {
	// bf_PT++
	// }
...
func (r *Replica) fastClock() {
	for !r.Shutdown {
		// time.Sleep(time.Duration(r.batchWait) * time.Millisecond) // batchWait ms
		time.Sleep(5 * time.Millisecond) // 5 ms
		fastClockChan <- true
	}
}
...
var conflicted, weird, slow, fast int
...
func (r *Replica) run() {
	...
	// 	fmt.Printf("--- Batching is Enabled \n")
	// 	go r.fastClock()
	// }
	...
	for !r.Shutdown {
		...
		case <-r.OnClientConnect:
			dlog.Printf("weird %d; conflicted %d; slow %d; fast %d\n", weird, conflicted, slow, fast)
			weird, conflicted, slow, fast = 0, 0, 0, 0
